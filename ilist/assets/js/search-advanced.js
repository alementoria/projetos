/**
 * DyList — Entrada Inteligente Unificada v5.0
...
class SmartSearch {
  ...
  async _handleInput(value) {
    ...
    if (!this._checkApiKey(value)) return;
  ...
  _addSelected() {
    ...
    if (isSelectAll) {
      ...
      const ids = Array.from(
        document.querySelectorAll('#smartSearchTabContent .video-checkbox:checked')
  ...
  _showBatchModal(selected) {
      ...
      <div style="background:#0d1b2a;border:1px solid rgba(255,153,0,0.15);border-radius:16px;padding:24px;max-width:400px;width:95%;color:#f1f5f9;position:relative;overflow:hidden;" onclick="event.stopPropagation()">
        ...
        <div style="background:#060B14;border-radius:8px;padding:14px;margin-bottom:18px;">
          <div style="display:flex;align-items:center;gap:12px;">
            <input type="range" id="batchSlider" min="1" max="${total}" value="${Math.min(50,total)}"
              style="flex:1;accent-color:#FF9900;"
              oninput="document.getElementById('batchVal').textContent=this.value">
            <span id="batchVal" style="font-size:20px;font-weight:800;color:#FF9900;min-width:36px;text-align:right;">${Math.min(50,total)}</span>
          </div>
    ...
    modal.querySelector('#batchOk').onclick = () => {
      const n = parseInt(document.getElementById('batchSlider').value);
      modal.remove();
  ...
  _checkApiKey(pendingValue = null) {
    this.API_KEY = localStorage.getItem('youtube_api_key');
    if (!this.API_KEY) {
      this._pendingSearch = pendingValue; // guarda para retentar depois de salvar
  ...
  _createApiKeyModal() {
    ...
    // IDs prefixados com "ss_" para não colidir com sidebar.js ou index.html
    modal.innerHTML = `
      <div style="background:#0d1b2a;border:1px solid rgba(255,153,0,0.15);border-radius:18px;padding:28px;width:90%;max-width:460px;position:relative;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,0.7);" onclick="event.stopPropagation()">
        ...
        <input type="password" id="ss_apiKeyInput" placeholder="Cole sua API Key aqui..."
        ...
        <button id="ss_saveApiKeyBtn" style="width:100%;background:linear-gradient(135deg,#FF9900,#f59e0b);color:#000;border:none;padding:12px;border-radius:8px;font-size:14px;font-weight:800;font-family:var(--sans);cursor:pointer;margin-left:0;margin-bottom:8px;">
          💾 Salvar e Buscar
        </button>
        <button id="ss_cancelApiKeyBtn" style="width:100%;background:#1e293b;color:#94a3b8;border:1px solid rgba(255,255,255,0.07);padding:10px;border-radius:8px;font-size:13px;font-family:var(--sans);cursor:pointer;margin-left:0;">
          Cancelar
        </button>
      </div>
    `;
    ...
    modal.querySelector('#ss_saveApiKeyBtn').onclick = () => {
      const input = modal.querySelector('#ss_apiKeyInput');
      const key   = input.value.trim();
      if (!key) { input.style.borderColor='#ef4444'; return; }
      // Salva em todas as chaves conhecidas para garantir compatibilidade
      localStorage.setItem('youtube_api_key', key);
      this.API_KEY = key;
      input.style.borderColor = '#10b981';
      modal.style.display = 'none';
      input.value = '';
      showQuickToast('✅ API Key salva! Iniciando busca...');
      // Retentar a última busca pendente automaticamente
      if (this._pendingSearch) {
        const pending = this._pendingSearch;
        this._pendingSearch = null;
        setTimeout(() => this._handleInput(pending), 300);
      }
    };

    modal.querySelector('#ss_cancelApiKeyBtn').onclick = () => {
      modal.style.display='none';
      this._pendingSearch = null;
    };

    modal.querySelector('#ss_apiKeyInput').addEventListener('keydown', e => {
      if (e.key === 'Enter') modal.querySelector('#ss_saveApiKeyBtn').click();
    });
  }
...
setTimeout(() => {
  ...
  console.log('%c🔍 DyList Smart Input ativo', 'color:#FF9900;font-weight:800;font-size:13px;');
}, 500);